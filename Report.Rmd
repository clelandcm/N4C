...
```{r}
table1(~ ., data = n4c[pid %in% Enrolled & pid %in% DT$pid[!is.na(DT$bl_date)] & 
...
```{r}
table1(~ Gender_Identity, data = gendID_m[pid %in% Enrolled & pid %in% DT$pid[!is.na(DT$bl_date)],])
```

### Sexual Identity

```{r}
table1(~ Sexual_Orientation, data = sexor_m[pid %in% Enrolled & pid %in% DT$pid[!is.na(DT$bl_date)],])
```
...
sexmin[, Enrolled := pid %in% Enrolled & pid %in% DT$pid[!is.na(DT$bl_date)]]
```
...
```{r}
GQI_m <- melt(DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), 
...
HIVdx <- merge(n4c[redcap_event_name == "Screen 1 (Arm 1: Screener)", .(pid, scr1_age)],
               n4c[pid %in% Enrolled & redcap_event_name == "Baseline Interview (Arm 2: Assessments)" & !is.na(bl_date), 
...
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "bl_outcome\\d"])
```
...
```{r}
socsup <- DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "pid|bl_mos\\d"]
...
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "bl_mos\\d"])
```
...
```{r}
edd <- DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "pid|discrim\\d+$"]
...
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "bl_discrim\\d+$"])
```
...
```{r}
stig <- DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "pid|bl_stigma\\d"]
...
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "bl_stigma\\d"])
```
...
```{r}
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "bl_clientcare\\d"])
```

## HIV Care Satisfaction

```{r}
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "bl_spns\\d"])
```
...
```{r}
tprov <- DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "pid|bl_trust\\d"]
...
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "bl_trust\\d"])
```
...
```{r}
ACES_m <- DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date), .SD, .SDcols = names(DT) %like% "pid|bl_aces\\d"]
...
```{r}
table1(~ drug_accept_test, data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date),])
```
...
```{r}
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date) & drug_accept_test == "Yes (Accepted)", .SD, .SDcols = names(DT) %like% "^drug_sample1_test"])
```

### Second Sample

```{r, eval = any(!is.na(n4c$drug_sample2) & n4c$drug_sample2 == "Yes")}
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date) & drug_sample2 == "Yes", .SD, .SDcols = names(DT) %like% "^drug_sample2_test"])
```
...
```{r, results='asis'}
mytab4 <- gtsummary::tbl_cross(wide_VL[pid %in% Enrolled & 
...
```{r, results='asis'}
mytab5 <- gtsummary::tbl_cross(wide_VL[pid %in% Enrolled & 
                                       !is.na(BL_Undetectable) &
                                       BL_Undetectable == "No",], 
                               row = FU1_Undetectable, 
                               col = FU2_Undetectable,
...
```{r}
change_VL_status <- wide_VL[pid %in% Enrolled &
...
```{r}
longit_cohort <- wide_VL[pid %in% Enrolled] |> 
...
```{r, eval=FALSE}
consort_plot(wide_VL[pid %in% Enrolled],
...
mytab4 <- wide_VL[pid %in% Enrolled,] |> 
  tbl_strata(
