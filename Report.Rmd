...
```{r, echo=FALSE}
Enrolled <- setdiff(Enrolled, wform$pid)
```
...
enroll_new[`Cumulative Target` >= 270, `Cumulative Target` := 270]
...
ggplot(enroll_new_m[type %in% c('Cumulative Enrolled','Cumulative Target'),], 
  ...
  geom_path() +
  scale_y_continuous(limits=c(0,figtop), breaks=seq(0,figtop,15)) +
...
```{r}
DT <- Reduce(function(x, y) merge(x, y, intersect(names(x), names(y)), all = TRUE), 
                 list(DT, 
...
label(DT$Any_Moderate_High_Risk) <- "Any Moderate or High Risk Substance Use"
label(DT$Any_Moderate_High_Risk_NoTOB) <- "Any Moderate or High Risk Substance Use, Excluding Tobacco"
label(DT$Any_Moderate_High_Risk_NoTOBCANALC) <- "Any Moderate or High Risk Substance Use, Excluding Tobacco, Alcohol, and Cannabis"
```
...
```{r}
table1(~ ., data = DT[pid %in% Enrolled & redcap_event_name == "baseline_interview_arm_2" & !is.na(bl_date),
                      .SD, .SDcols = names(DT) %like% "Any_Moderate_"])
```
